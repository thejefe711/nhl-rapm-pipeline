...
WHERE player_id = {slavin_id}
  AND season = '{season}'
  AND (metric_name LIKE 'xg%' OR metric_name LIKE 'corsi%' OR metric_name LIKE 'blocked%')
ORDER BY metric_name
"""

res = conn.execute(query).df()
print(f"Jaccob Slavin Key Metrics ({season}):")
for _, row in res.iterrows():
    print(f"{row.metric_name}: {row.value:.4f} (Rank {row.final_rank}/{row.total_players})")
